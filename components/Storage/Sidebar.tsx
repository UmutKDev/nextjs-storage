...
export default function Sidebar({ className, onCreateFolder, onUpload }: SidebarProps) {
  ...
  return (
    <div className={cn("w-64 flex flex-col h-full border-r bg-card/30 backdrop-blur-xl p-6 gap-6", className)}>
      {/* New Action Button */}
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button size="lg" className="w-full justify-start gap-2 shadow-lg shadow-primary/20 rounded-2xl h-12 text-base font-semibold">
            <Plus className="w-5 h-5" />
            Yeni Ekle
          </Button>
        </DropdownMenuTrigger>
        <DropdownMenuContent align="start" className="w-56 p-2 rounded-xl">
          <DropdownMenuItem onClick={onUpload} className="gap-2 p-3 rounded-lg cursor-pointer">
      ...
      <div className="mt-auto pt-6 border-t border-border/50">
        <div className="bg-muted/30 p-5 rounded-3xl border border-border/50 shadow-sm">
          ...
          <Button variant="default" size="sm" className="w-full mt-5 rounded-xl text-xs h-9 shadow-md">
            Yükselt
          </Button>
        </div>
      </div>
